...
public class AsteriskServerImpl implements AsteriskServer, ManagerEventListener {
	...
	private synchronized void initializeIfNeeded()
		...
		if (eventConnection.getState() == ManagerConnectionState.INITIAL
			...
			try {
				...
				throw new ManagerCommunicationException("Unable to login: "
						+ e.getMessage(), e);
			}
		}
	...
	public AsteriskChannel originate(OriginateAction originateAction)
		...
		if (responseEventIterator.hasNext()) {
			...
			if (responseEvent instanceof OriginateResponseEvent) {
				...
				try {
					channel = awaitChannelById(uniqueId,
	...
	public void originateAsync(OriginateAction originateAction,
		...
		if (cb != null) {
			...
			callbackData = new OriginateCallbackData(originateAction,
					DateUtil.getDate(), cb);
			// register callback
		...
		if (response != null && response instanceof ManagerError) {
			if (cb != null) {
				...
				LiveException cause = new ManagerErrorException(
						(ManagerError) response);
				cb.onFailure(cause);
			}
		}
	}
	...
	AsteriskChannel awaitChannelById(String id, long timeout)
			throws ManagerCommunicationException, InterruptedException {
		initializeIfNeeded();
	...
	public synchronized String getVersion()
		...
		initializeIfNeeded();
		if (version == null) {
			if (eventConnection.getVersion().isAtLeast(
			...
			if (response instanceof CommandResponse) {
				...
				logger.error("Response to CommandAction(\"" + command
	...
	public Collection<Voicemailbox> getVoicemailboxes()
		...
		if (eventConnection.getVersion()
				.isAtLeast(AsteriskVersion.ASTERISK_1_6)) {
			response = sendAction(new CommandAction(
					SHOW_VOICEMAIL_USERS_1_6_COMMAND));
		} else {
			response = sendAction(new CommandAction(
		...
		for (Voicemailbox voicemailbox : voicemailboxes) {
			...
			if (response instanceof MailboxCountResponse) {
				...
				logger.error("Response to MailboxCountAction was not a MailboxCountResponse but "
						+ response);
			}
		}
	...
	public ConfigFile getConfig(String filename)
		...
		if (!(response instanceof GetConfigResponse)) {
			throw new ManagerCommunicationException(
					"Response to GetConfigAction(\"" + filename
							+ "\") was not a CommandResponse but " + response,
					null);
		}
		...
		final Map<Integer, String> categoryMap = getConfigResponse
				.getCategories();
		for (Map.Entry<Integer, String> categoryEntry : categoryMap.entrySet()) {
			final List<String> lines;
			final Map<Integer, String> lineMap = getConfigResponse
					.getLines(categoryEntry.getKey());
	...
	public void onManagerEvent(ManagerEvent event) {
		...
		// Handle queue related event
			...
			queueManager
					.handleQueueMemberPenaltyEvent((QueueMemberPenaltyEvent) event);
			...
			queueManager
					.handleQueueMemberPausedEvent((QueueMemberPausedEvent) event);
		}
		// >>>>>> AJ 94
		// Handle meetMeEvents
	...
	public void shutdown() {
		if (eventConnection != null
				&& (eventConnection.getState() == ManagerConnectionState.CONNECTED || eventConnection
